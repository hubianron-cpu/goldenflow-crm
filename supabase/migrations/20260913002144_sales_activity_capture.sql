...
-- Optional snapshots preserve old activity rows and the existing agent read contract.
alter table public.lead_sales_activities
  add column next_step_mode text check (next_step_mode in ('keep', 'none', 'schedule')),
  add column next_action_type text check (next_action_type in ('call', 'message', 'meeting', 'follow-up')),
  add column next_action_date timestamptz;

create function public.record_lead_sales_activity(
  p_request_id uuid,
  p_lead_id uuid,
  p_outcome text,
  p_summary text,
...
begin
  if v_user is null or not exists (
    ...
    where s.user_id = v_user
      and (s.status = 'active' or (s.status = 'trial' and s.trial_end_at > now()))
  ) then
  ...
  -- Do not infer call direction or who initiated a callback request from the outcome alone.
  ...
  -- Serialize submissions per owned lead; RLS remains active under SECURITY INVOKER.
  ...
  -- No status transition, task mutation, or inferred contact for an unanswered call/internal note.
  -- Terminal leads are history-only. Lead follow-up fields are the existing scheduling model.
...
revoke all on function public.record_lead_sales_activity(uuid, uuid, text, text, text, text, timestamptz) from public, anon;
grant execute on function public.record_lead_sales_activity(uuid, uuid, text, text, text, text, timestamptz) to authenticated;
