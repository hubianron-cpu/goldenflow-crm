import { BadgeDollarSign, Crown, LogOut, Menu } from "lucide-react";
  ...
  return (
    <div className="relative min-h-screen max-w-full overflow-x-clip px-3 py-4 sm:px-5 lg:px-6">
      ...
      <div className="relative mx-auto grid w-full max-w-[1440px] min-w-0 gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="relative h-fit w-full max-w-full overflow-hidden rounded-[28px] border border-gold/15 bg-black/55 p-3 shadow-[0_28px_90px_rgba(0,0,0,0.42),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-xl lg:sticky lg:top-6 lg:min-h-[calc(100vh-3rem)] lg:p-5">
          ...
          <input aria-label="פתיחת תפריט ניווט" className="peer sr-only lg:hidden" id="mobile-navigation-toggle" type="checkbox" />
          <label
            className="relative flex min-h-12 cursor-pointer items-center justify-between gap-3 rounded-2xl border border-gold/20 bg-white/[0.035] px-3 py-2 text-sm font-bold text-white transition hover:border-gold/35 lg:hidden"
            htmlFor="mobile-navigation-toggle"
          >
            <span className="flex min-w-0 items-center gap-3">
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-gold text-black">
                <BadgeDollarSign className="h-4 w-4" />
              </span>
              <span className="min-w-0 break-words">מרכז השליטה של העסק שלך</span>
            </span>
            <Menu aria-hidden="true" className="h-5 w-5 shrink-0 text-gold-soft" />
          </label>
