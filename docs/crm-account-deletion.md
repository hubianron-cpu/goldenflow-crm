...
Read-only Production inspection on 2026-09-24 confirmed that `business_center_content_items.user_id` currently references `auth.users` with `ON DELETE NO ACTION`. Production also has `business_center_lead_attributions.content_item_id -> business_center_content_items.id ON DELETE RESTRICT`. Neither account-deletion migration has been applied to Production. `client_activations` is absent there, so the Activation branch of the earlier migration would be skipped. Do not run Production account deletion until both migrations have received separate approval, their schema effects are verified, and the non-database gates below are satisfied.
